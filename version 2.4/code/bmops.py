    ...
    def execute(self, context):
        ...
        try:
            ...
            if self.option == "shade_smooth":
                smooth = [pool.shade_smooth] * len(ob.data.polygons)
...
def anicalc_factors(val):
    return set(
        f for i in range(1, int(val**0.5) + 1) if not val % i
        for f in (i, val // i)
    )
...
def aniact_value_list(flag, v1, v2, mirror, cycles, loop):
    ...
    if not flag or (v1 == v2):
        return [v1] * loop
    if not mirror:
        lst = linear_list(1 / (loop - 1), False, loop)
...
def aniact_path_edit_dict(path, loop):
    ...
    def path_dim_list(pdim, anpdim, dims):
        p = []
        for i in range(dims):
            p.append(
                aniact_value_list(adim, pdim[i], anpdim[i], dim_mirr, dim_cycl, loop)
            )
        return p

    p_n = path.provider
    if p_n == "custom":
        p = path_dim_list(path.pathed.cust_dim, path.ani_dim_3d, 3)
        d["dim"] = [[p[0][i], p[1][i], p[2][i]] for i in range(loop)]
        return d
    if p_n == "polygon":
        p = path_dim_list(path.pathed.pol_dim, path.ani_dim_2d, 2)
        d["dim"] = [[p[0][i], p[1][i]] for i in range(loop)]
        return d
    afac = path.ani_fac
    fac_mirr = path.ani_fac_mirror.active
    fac_cycl = path.ani_fac_mirror.cycles
    if p_n == "line":
        d["dim"] = aniact_value_list(
            adim, path.pathed.lin_dim, path.ani_lin_dim, dim_mirr, dim_cycl, loop
        )
        d["fac"] = aniact_value_list(
            afac, path.pathed.lin_exp, path.ani_lin_exp, fac_mirr, fac_cycl, loop
        )
        return d
    if p_n == "arc":
        d["dim"] = aniact_value_list(
            adim, path.pathed.arc_dim, path.ani_arc_dim, dim_mirr, dim_cycl, loop
        )
        d["fac"] = aniact_value_list(
            afac, path.pathed.arc_fac, path.ani_arc_fac, fac_mirr, fac_cycl, loop
        )
        return d
    if p_n == "spiral":
        d["dim"] = aniact_value_list(
            adim, path.pathed.spi_dim, path.ani_spi_dim, dim_mirr, dim_cycl, loop
        )
        d["fac"] = aniact_value_list(
            afac, path.pathed.spi_revs, path.ani_spi_revs, fac_mirr, fac_cycl, loop
        )
        return d
    if p_n == "ellipse":
        p = path_dim_list(path.pathed.ell_dim, path.ani_dim_2d, 2)
        d["dim"] = [[p[0][i], p[1][i]] for i in range(loop)]
        d["fac"] = aniact_value_list(
            afac,
            path.pathed.ellstep_val,
            path.ani_ellstep_val,
            fac_mirr,
            fac_cycl,
            loop,
        )
    ...
    if p_n == "wave":
        d["dim"] = aniact_value_list(
            adim, path.pathed.wav_dim, path.ani_wav_dim, dim_mirr, dim_cycl, loop
        )
        d["fac"] = aniact_value_list(
            afac, path.pathed.wav_amp, path.ani_wav_amp, fac_mirr, fac_cycl, loop
        )
        d["fac2"] = aniact_value_list(
            afac2, path.pathed.wav_frq, path.ani_wav_frq, fac2_mirr, fac2_cycl, loop
        )
        d["fac3"] = aniact_value_list(
            afac3, path.pathed.wav_pha, path.ani_wav_pha, fac3_mirr, fac3_cycl, loop
        )
        return d
    afac4 = path.ani_fac4
    fac4_mirr = path.ani_fac4_mirror.active
    fac4_cycl = path.ani_fac4_mirror.cycles
    p = path_dim_list(path.pathed.hel_dim, path.ani_dim_2d, 2)
    d["dim"] = [[p[0][i], p[1][i]] for i in range(loop)]
    d["fac"] = aniact_value_list(
        afac, path.pathed.hel_len, path.ani_hel_len, fac_mirr, fac_cycl, loop
    )
    d["fac2"] = aniact_value_list(
        afac2, path.pathed.hel_fac, path.ani_hel_fac, fac2_mirr, fac2_cycl, loop
    )
    d["fac3"] = aniact_value_list(
        afac3, path.pathed.hel_stp, path.ani_hel_stp, fac3_mirr, fac3_cycl, loop
    )
    d["fac4"] = aniact_value_list(
        afac4, path.pathed.hel_pha, path.ani_hel_pha, fac4_mirr, fac4_cycl, loop
    )
    return d
...
def aniact_prof_edit_dict(prof, loop):
    ...
    def prof_dim_list(p_dim):
        p = []
        for i in range(2):
            p.append(
                aniact_value_list(
                    adim, p_dim[i], prof.ani_epc_dim[i], dim_mirr, dim_cycl, loop
                )
            )
        return p

    p_n = prof.provider
    if p_n in {"polygon", "custom"}:
        p_dim = prof.profed.pol_dim if p_n == "polygon" else prof.profed.cust_dim
        p = prof_dim_list(p_dim)
        d["dim"] = [[p[0][i], p[1][i]] for i in range(loop)]
        return d
    afac = prof.ani_fac
    ...
    if p_n == "line":
        d["dim"] = aniact_value_list(
            adim, prof.profed.lin_dim, prof.ani_lin_dim, dim_mirr, dim_cycl, loop
        )
        d["fac"] = aniact_value_list(
            afac, prof.profed.lin_exp, prof.ani_lin_exp, fac_mirr, fac_cycl, loop
        )
        return d
    if p_n == "arc":
        d["dim"] = aniact_value_list(
            adim, prof.profed.arc_dim, prof.ani_arc_dim, dim_mirr, dim_cycl, loop
        )
        d["fac"] = aniact_value_list(
            afac, prof.profed.arc_fac, prof.ani_arc_fac, fac_mirr, fac_cycl, loop
        )
        return d
    if p_n == "ellipse":
        p = prof_dim_list(prof.profed.ell_dim)
        d["dim"] = [[p[0][i], p[1][i]] for i in range(loop)]
        d["fac"] = aniact_value_list(
            afac,
            prof.profed.ellstep_val,
            prof.ani_ellstep_val,
            fac_mirr,
            fac_cycl,
            loop,
        )
    ...
    d["dim"] = aniact_value_list(
        adim, prof.profed.wav_dim, prof.ani_wav_dim, dim_mirr, dim_cycl, loop
    )
    d["fac"] = aniact_value_list(
        afac, prof.profed.wav_amp, prof.ani_wav_amp, fac_mirr, fac_cycl, loop
    )
    d["fac2"] = aniact_value_list(
        afac2, prof.profed.wav_frq, prof.ani_wav_frq, fac2_mirr, fac2_cycl, loop
    )
    d["fac3"] = aniact_value_list(
        afac3, prof.profed.wav_pha, prof.ani_wav_pha, fac3_mirr, fac3_cycl, loop
    )
    return d


def aniact_plocval_list(inst, loop):
    return aniact_value_list(
        inst.ani_fac.active,
        inst.fac,
        inst.ani_fac.fac,
        inst.ani_fac.mirror.active,
        inst.ani_fac.mirror.cycles,
        loop,
    )
...
def aniact_blndval_list(item, loop):
    return aniact_value_list(
        ...
        item.fac,
        item.ani_fac_val,
        item.ani_fac_mirror.active,
        item.ani_fac_mirror.cycles,
        loop,
    )
...
class PTDBLNPOPM_OT_anim_action(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            pop = ModPOP.new_pop_instance(path, prof)
            pop.meshrot_active = meshrot.active
            if meshrot.active:
                pop.mesh_rotate(meshrot.axis, meshrot.angle, meshrot.pivot)
            pop.pathrot_active = pathrot.active
            if pathrot.active:
                pop.path_rotate(pathrot.axis, pathrot.angle)
            if profrot.active:
                pop.prof_rotate(profrot.roll, profrot.twist, profrot.follow_limit)
            kloc = []
            for i in range(loop):
                if path_flag:
