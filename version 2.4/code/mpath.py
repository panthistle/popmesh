...
class Line:
    ...
    def get_locs(self):
        ...
        base = -0.5 * vdim
        locs = [base + vdim * t for t in fvs]
        if self.reverse:
            if self.closed:
                locs = locs[:1] + locs[1:][::-1]
            else:
                locs.reverse()
        sid = self.offset
...
class Wave:
    """sine wave (XY, from x=dim/2, y=0): path, profile"""
    ...
    def get_locs(self):
        ...
        locs = [
            Vector((base + dtx * i, self.amp * math.sin(dty * i + self.pha), 0))
            for i in range(npts)
        ]
        if self.reverse:
            if self.closed:
...
class Arc:
    ...
    def get_locs(self):
        ...
        if s == 0:
            dt = w / (npts - 1)
            locs = [Vector(((w / 2) - dt * i, 0, 0)) for i in range(npts)]
            if self.reverse:
                if self.closed:
                    locs = locs[:1] + locs[1:][::-1]
                else:
...
class Helix:
    """helix (XYZ, counterclock from x=dim[0]/2, y=0, z=length/2): path"""
    ...
    def get_locs(self):
        ...
        if self.invert:
            rls.reverse()
        dt = 2 * math.pi * self.steps / (npts - 1)
...
class Custom:
    ...
    def update(self, dct):
        ...
        self._udim = tuple(i for i in dct["user_dim"])
        self.dim = tuple(i for i in dct["cust_dim"])
