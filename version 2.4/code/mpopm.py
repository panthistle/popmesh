...
# ------------------------------------------------------------------------------
#
# -------------------- RANDOM FACTOR ANIMATION UPDATES -------------------------
...
def new_pop_instance(path, prof):
    ...
    return ModDATA.PopEx(path.to_dct(), prof.to_dct())
...
def pop_update_all(pop, pool):
    pop.meshrot_active = pool.meshrot.active
    if pool.meshrot.active:
        pop.mesh_rotate(pool.meshrot.axis, pool.meshrot.angle, pool.meshrot.pivot)
    pop.pathrot_active = pool.pathrot.active
    if pool.pathrot.active:
        pop.path_rotate(pool.pathrot.axis, pool.pathrot.angle)
    if pool.profrot.active:
        pop.prof_rotate(
            pool.profrot.roll, pool.profrot.twist, pool.profrot.follow_limit
        )
    for item in pool.pathloc:
        if item.active:
...
def scene_update(scene, setup="none"):
    ...
    pop = new_pop_instance(pool.path, pool.prof)
    rings = pop.rings
    rpts = pop.rpts
    if setup == "all":
